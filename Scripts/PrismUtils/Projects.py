...
class Projects(object):
    ...
    @err_catcher(name=__name__)
    def getRequiredProjectFolders(self, projectName):
        basePaths = [
            r"\\172.18.20.12\Project Run",
            r"\\172.18.20.12\Rendering",
            r"\\172.18.20.12\M&P",
        ]
        return [os.path.join(basePath, projectName) for basePath in basePaths]

    @err_catcher(name=__name__)
    def createRequiredProjectFolders(self, projectName, parent=None):
        errors = []
        for folderPath in self.getRequiredProjectFolders(projectName):
            try:
                os.makedirs(folderPath)
            except FileExistsError:
                pass
            except Exception as e:
                errors.append((folderPath, str(e)))

        if errors:
            details = "\n".join(
                ["%s\n%s" % (folderPath, error) for folderPath, error in errors]
            )
            msg = (
                "The project was created, but Prism could not create all required shared folders:\n\n%s"
                % details
            )
            self.core.popup(msg, parent=parent)
    ...
    @err_catcher(name=__name__)
    def createProject(
        self, name, path, settings=None, preset="Default", image=None, structure=None, parent=None, createRequiredFolders=True
    ):
        prjName = name
        prjPath = path.strip(" ")
        settings = settings or {}
        ...
        if createRequiredFolders:
            self.createRequiredProjectFolders(prjName, parent=parent)
