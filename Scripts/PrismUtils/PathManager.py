...
class PathManager(object):
    def __init__(self, core):
        ...
        # Compatibility defaults for the simplified artist workflow.
        self.defaultDepartment = "main"
    ...
    def normalizeDepartment(self, value, context=None, reason=None):
        ...
        department = self.defaultDepartment
        logger.debug(
            "Injecting default department '%s'%s",
            department,
    ...
    @err_catcher(name=__name__)
    def normalizeTask(self, value, context=None, reason=None):
    ...
    @err_catcher(name=__name__)
    def getDefaultWorkContext(self, context=None, reason=None):
        context = (context or {}).copy()
        context["department"] = self.normalizeDepartment(
            context.get("department"), context=context, reason=reason
        )
        context["task"] = self.normalizeTask(
            context.get("task"), context=context, reason=reason
        )
        return context

    @err_catcher(name=__name__)
    def isDefaultDepartmentTask(self, department=None, task=None):
        return (
            self.normalizeDepartment(department) == self.defaultDepartment
    ...
    def getSimplifiedOutputRoot(self, entity, projectPath=None):
        ...
        parts = [projectPath, "Output", self.simplifiedShotStage]
    ...
    @err_catcher(name=__name__)
    def getSimplifiedScenefileFolder(self, entity, projectPath=None):
        root = self.getSimplifiedOutputRoot(entity, projectPath=projectPath)
        if not root:
            return ""
    ...
    def getSimplifiedEntityDataFromPath(self, path, projectPath=None):
        ...
        def _entity_parts(rootPath):
            relPath = os.path.relpath(globalPath, rootPath)
        ...
        if globalPath.startswith(assetRoot + os.sep) or globalPath == assetRoot:
            entityParts = _entity_parts(assetRoot)
            assetPath = "/".join([p for p in entityParts if p and p != "."])
            if assetPath:
                return {
                    "type": "asset",
                    "entityType": "asset",
                    "asset_path": assetPath,
                    "asset": os.path.basename(assetPath),
                    "project_path": projectPath,
                }

        if globalPath.startswith(shotRoot + os.sep) or globalPath == shotRoot:
            entityParts = _entity_parts(shotRoot)
            entityParts = [p for p in entityParts if p and p != "."]
            if entityParts:
                shot = entityParts[-1]
                sequence = "/".join(entityParts[:-1])
                data = {
                    "type": "shot",
                    "entityType": "shot",
                    "shot": shot,
    ...
    @err_catcher(name=__name__)
    def getSimplifiedScenefileData(self, path, projectPath=None):
        data = self.getSimplifiedEntityDataFromPath(path, projectPath=projectPath)
        if not data:
            return {}

        if os.path.basename(os.path.dirname(path)) != "Scenefiles":
            return {}
        ...
        data["version"] = versionMatch.group("version")
        data["extension"] = ext
        data["department"] = self.defaultDepartment
        data["task"] = self.defaultTask

        if data["type"] == "asset":
            if versionMatch.group("name") != data.get("asset"):
                return {}
        else:
            expected = data["shot"]
            if data.get("sequence"):
                expected = "%s_%s" % (data["sequence"].replace("\\", "/").split("/")[-1], data["shot"])

            if versionMatch.group("name") != expected:
                return {}

        logger.debug("Detected simplified scenefile path: %s", path)
        return data
    ...
    def getEntityPath(
        ...
        context = entity.copy()
        if (
            self.isSimplifiedArtistWorkflowEnabled()
            and entity.get("type") in ["asset", "shot"]
            and step
            and self.isDefaultDepartmentTask(step, category)
        ):
            path = self.getSimplifiedScenefileFolder(entity)
            if reqEntity == "step":
    ...
    def generateScenePath(
        ...
        department = self.normalizeDepartment(
        ...
        if (
            self.isSimplifiedArtistWorkflowEnabled()
            and entity.get("type") in ["asset", "shot"]
            and self.isDefaultDepartmentTask(department, task)
        ):
            folder = self.getSimplifiedScenefileFolder(entity, projectPath=self.core.projectPath)
            if entity["type"] == "asset":
                name = os.path.basename(entity["asset_path"])
            else:
                shotLabel = entity.get("shot", "")
