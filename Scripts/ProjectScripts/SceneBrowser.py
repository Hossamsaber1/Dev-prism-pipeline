...
class SceneBrowser(QWidget, SceneBrowser_ui.Ui_w_sceneBrowser):
    ...
    def loadLayout(self):
        ...
        if self.core.paths.isSimplifiedArtistWorkflowEnabled():
            # Compatibility patch: keep the underlying department/task browser
            # model alive, but hide it from the normal artist workflow.
            self.w_departmentsHeader.setVisible(False)
            self.w_tasksHeader.setVisible(False)
            self.lw_departments.setVisible(False)
            self.lw_tasks.setVisible(False)
    ...
    def navigate(self, data):
        ...
        if self.core.paths.isSimplifiedArtistWorkflowEnabled():
            data = self.core.paths.getDefaultWorkContext(
                data, reason="scene browser navigation"
            )
    ...
    def entityCreated(self, data):
        if data.get("type", "") == "asset":
            if (
                data.get("action") == "next"
                and not self.core.paths.isSimplifiedArtistWorkflowEnabled()
            ):
    ...
    def refreshDepartments(self, restoreSelection=False):
        if self.core.paths.isSimplifiedArtistWorkflowEnabled():
            defaultDepartment = self.core.paths.normalizeDepartment(
                None, reason="scene browser hidden department"
            )
            ...
            item = QListWidgetItem(defaultDepartment)
            item.setData(Qt.UserRole, defaultDepartment)
    ...
    def refreshTasks(self, restoreSelection=False):
        if self.core.paths.isSimplifiedArtistWorkflowEnabled():
            defaultTask = self.core.paths.normalizeTask(
                None, reason="scene browser hidden task"
            )
            ...
            self.lw_tasks.clear()
            self.lw_tasks.addItem(QListWidgetItem(defaultTask))
    ...
    def getCurrentDepartment(self):
        ...
        if not item:
            return self.core.paths.normalizeDepartment(
                None, reason="scene browser current department"
            )
    ...
    def getCurrentTask(self):
        ...
        if not item:
            return self.core.paths.normalizeTask(
                None, reason="scene browser current task"
            )
